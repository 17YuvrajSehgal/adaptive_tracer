...
%% Abstract
\begin{abstract}
We introduce LMAT, a Language Model-based Adaptive Tracing framework designed for intelligent DevOps pipelines requiring granular observability without excessive monitoring overhead. LMAT leverages a multi-task architecture to jointly predict kernel event sequences and classify event durations, thereby capturing both control-flow and temporal dynamics. By continuously comparing live trace data against model predictions, LMAT automatically signals deviations, dynamically adjusting trace granularity only when needed. This approach significantly reduces trace volume, along with associated energy and storage costs, achieving a 70.6\% reduction in our experiments.

Additionally, LMAT utilizes prediction discrepancies to drive an efficient root-cause classifier, mapping detected anomalies directly to their potential fault sources and providing actionable feedback for operations teams. Evaluated against an extensive corpus of Linux kernel traces, including standard workloads and two newly introduced duration-centric noise scenarios, LMAT demonstrates up to \textbf{97.7\%} accuracy in anomaly detection and root-cause identification, surpassing state-of-the-art methods relying solely on event sequences. Our findings illustrate that LMAT offers a practical, sustainable, and AI-driven solution for adaptive tracing, ideally suited for modern DevOps environments striving for balanced performance, reliability, and resource efficiency.
\end{abstract}
...
\begin{highlights}
\item Proposes LMAT, a language model-based adaptive tracing approach tailored for intelligent DevOps observability.
\item Dynamically adjusts tracing granularity by detecting deviations in live kernel event sequences and durations.
\item Achieves 97.7\% anomaly detection accuracy while reducing trace data volume by 70.6\%, enhancing observability with minimal overhead.
...
%% Keywords
\begin{keyword}
Adaptive Tracing\sep Language Model\sep Root Cause Analysis\sep Change Detection\sep Trace Duration Modeling\sep Observability\sep Intelligent DevOps
\end{keyword}
...
Our experiments demonstrate LMAT’s effectiveness in adaptively tracing system behavior, capturing abrupt changes with only 3.2\% event loss and reducing collected trace data by 70.6\%. Moreover, LMAT accurately identifies anomaly scenarios with 97.7\% accuracy, significantly outperforming previous state-of-the-art methods that rely solely on event sequence modeling. This improvement underscores LMAT’s ability to efficiently balance comprehensive monitoring with minimal resource overhead.
...
\begin{figure}[tbp]
    ...
    \caption{\textcolor{blue}{Timestamp-ordered event stream with entry/exit records for three event types (\(E_1,E_2,E_3\)).
...
\textcolor{red}{In the implemented LMAT pipeline, syscall entry and exit records are further normalized by storing the base syscall identity once and encoding the execution phase separately through an \textit{entry/exit indicator}. This design lets the model share statistical strength across the two phases of the same syscall while still distinguishing call initiation from completion.}
...
\textcolor{red}{At deployment time, LMAT computes one novelty score per sequence. In single-task configurations, this score is the corresponding sequence-level loss. In the multi-task configuration, the event and duration losses are first normalized with Median Absolute Deviation statistics estimated from held-out in-distribution validation data and are then summed to produce a single decision score before thresholding.}
...
\begin{figure}[tbp]
    ...
    \caption{\textcolor{blue}{Comparison of incorrect prediction distributions across out-of-distribution sets for frequent events, showing the unique pattern each type of anomaly exhibits.}}
...
\subsection{\textcolor{blue}{Anomaly Generation Methodology}}

\textcolor{red}{For clarity, the Apache environment contains seven injected server-side faults, listed below, all applied under the same workload conditions used during normal operation (1,000 HTTP requests per second).}

\textcolor{blue}{To evaluate the model’s ability to detect anomalies, we introduced six types of controlled server-side faults, each designed to mimic common real-world issues. These anomalies were injected individually under the same workload conditions used during normal operation (1,000 HTTP requests per second) to ensure consistent system stress and comparability across experiments.}

\textcolor{blue}{The anomalies are as follows:}

\textcolor{red}{The Apache anomaly set contains seven conditions in total, corresponding exactly to the seven items listed below.}

\begin{itemize}
    \item \textcolor{blue}{\textbf{OPcache:} PHP's OPcache was turned off, preventing the reuse of precompiled bytecode and thereby increasing request latency. This behavior reflects a common development misconfiguration when caching is disabled and not re-enabled before deployment.}
    \item \textcolor{blue}{\textbf{Dump IO:} Apache’s \texttt{dump\_io} module was activated with maximum verbosity, resulting in excessive disk I/O from debug logging.}
    \item \textcolor{blue}{\textbf{Connection:} Apache’s concurrent connection threshold was lowered from 150 to 25, simulating a misconfiguration or limited-resource deployment.}
    \item \textcolor{blue}{\textbf{CPU:} Using the \texttt{stress-ng} tool to run intensive matrix multiplication operations, simulating resource contention from unexpected CPU-intensive tasks.}
    \item \textcolor{blue}{\textbf{Socket:} The persistent connection feature (KeepAlive) in Apache2 was disabled. This forces clients to reopen sockets for each request, increasing CPU utilization and latency, and representing a deployment optimized for memory-limited environments.}
    \item \textcolor{blue}{\textbf{Sysbench:} We used \texttt{sysbench} to create short bursts of many concurrent database requests. This increases the number of open connections and the amount of work MySQL must do at the same time, which in turn causes queueing delays and slower responses at the web tier.}
    \item \textcolor{blue}{\textbf{Bandwidth:} We intentionally limited the server’s network speed so responses had to travel over a “narrower pipe”. With less bandwidth, each request takes longer to send/receive, even when the CPU is idle, so overall page loads slow down due to the network alone.}
\end{itemize}

\textcolor{blue}{These injected conditions offer a diverse mix of CPU, I/O, and configuration-level anomalies, helping assess LMAT’s sensitivity to both subtle and significant changes in system behavior.}


\textcolor{red}{For Sock Shop, we used host-level anomaly injection while driving the application with the repository's load generator over realistic shopping actions. The four anomaly families are as follows:}

\begin{itemize}
    \item \textcolor{red}{\textbf{CPU stress:} We used \texttt{stress-ng} to saturate the available CPU resources, creating contention that propagates across the co-located microservices.}
    \item \textcolor{red}{\textbf{Disk stress:} We used \texttt{stress-ng} to generate heavy disk activity, increasing I/O wait time and affecting the storage-backed services in the application stack.}
    \item \textcolor{red}{\textbf{Memory stress:} We used \texttt{stress-ng} to create sustained memory pressure, inducing slower execution and interference across concurrently running services.}
    \item \textcolor{red}{\textbf{Network stress:} We used host-level traffic control to perturb network behavior, introducing communication slowdowns that propagate through inter-service interactions.}
\end{itemize}
...
\begin{figure}[tb]
    ...
    \caption{\textcolor{blue}{{Distribution shift of ``connect" syscall durations after 10 seconds of normal operation, demonstrating the need for duration category adjustments.}}}
...
\begin{table}[tb]
\caption{\textcolor{blue}{The definition and size of each subset of system calls, designed to assess the feasibility of system monitoring with a reduced set of events.}}
...
The implementation was carried out in Python using the Pytorch framework \cite{paszke2019pytorch}. We set the hyper-parameter $\lambda$ at 0.5. The selection of other hyper-parameters is oriented towards optimizing the change detection task, which is the ultimate objective. To minimize the impact of random variation, each test was performed three times with distinct seeds. The training process takes place on a server provided by the Digital Research Alliance of Canada~\footnote{https://alliancecan.ca}, equipped with two Intel Gold 6148 Skylake processors, four Nvidia V100SXM2 16G GPUs, and 64GB of RAM. \textcolor{red}{The Apache experiments were trained on this configuration.} \textcolor{red}{For the Sock Shop experiments, training was conducted on a separate Alliance compute allocation using one node, 12 CPU cores, one NVIDIA H100 GPU, and 32\,GB of memory.}
...
\begin{table*}[t]
\centering
\caption{\textcolor{red}{LSTM multitask OOD results across category settings.}}
...
\hline
\multirow{2}{*}{Dataset} & \multicolumn{5}{c|}{3 Categories} & \multicolumn{5}{c|}{5 Categories} & \multicolumn{5}{c|}{7 Categories} & \multicolumn{5}{c}{9 Categories} \\
 & F1 & AUROC & AUPR & Recall & Precision & F1 & AUROC & AUPR & Recall & Precision & F1 & AUROC & AUPR & Recall & Precision & F1 & AUROC & AUPR & Recall & Precision \\
\hline
CPU & 64.7 & 63.5 & 61.8 & 80.5 & 54.0 & 66.0 & \textbf{66.8} & \textbf{64.1} & 74.8 & \textbf{59.0} & 63.9 & 65.1 & 63.0 & 69.9 & 58.9 & \textbf{66.3} & 65.8 & 63.4 & \textbf{91.1} & 52.2 \\
Disk & 57.4 & 70.8 & 68.2 & 82.8 & 43.9 & 59.2 & 71.4 & 67.9 & 75.6 & 48.6 & \textbf{59.8} & 72.2 & 69.0 & 74.8 & \textbf{49.9} & 57.7 & \textbf{73.5} & \textbf{70.1} & \textbf{92.7} & 41.9 \\
Memory & 68.5 & 67.5 & 70.1 & 83.3 & 58.1 & \textbf{71.4} & \textbf{72.2} & \textbf{73.1} & 80.7 & \textbf{63.9} & 68.9 & 69.3 & 71.6 & 75.0 & 63.7 & 69.3 & 70.0 & 71.6 & \textbf{91.8} & 55.6 \\
Network & 60.0 & 74.6 & 65.8 & 89.4 & 45.1 & 63.5 & 76.3 & 68.1 & 84.9 & 50.7 & \textbf{65.0} & 77.5 & 67.7 & 85.6 & \textbf{52.4} & 58.4 & \textbf{80.0} & \textbf{72.3} & \textbf{96.0} & 42.0 \\
\hline
Average & 62.6 & 69.1 & 66.5 & 84.0 & 50.3 & \textbf{65.0} & 71.7 & 68.3 & 79.0 & 55.6 & 64.4 & 71.0 & 67.8 & 76.3 & \textbf{56.2} & 62.9 & \textbf{72.3} & \textbf{69.3} & \textbf{92.9} & 47.9 \\
\hline
\end{tabular}%
}
}
...
\begin{table*}[t]
\centering
\caption{\textcolor{red}{TRANSFORMER multitask OOD results across category settings.}}
...
\multirow{2}{*}{Dataset} & \multicolumn{5}{c|}{3 Categories} & \multicolumn{5}{c|}{5 Categories} & \multicolumn{5}{c|}{7 Categories} & \multicolumn{5}{c}{9 Categories} \\
 & F1 & AUROC & AUPR & Recall & Precision & F1 & AUROC & AUPR & Recall & Precision & F1 & AUROC & AUPR & Recall & Precision & F1 & AUROC & AUPR & Recall & Precision \\
\hline
CPU & 64.8 & 64.7 & 62.7 & 77.0 & 55.9 & \textbf{67.8} & \textbf{68.6} & \textbf{65.4} & \textbf{82.3} & \textbf{57.6} & 65.2 & 65.1 & 62.8 & 75.7 & 57.2 & 66.3 & 66.1 & 63.5 & 80.1 & 56.6 \\
Disk & 57.8 & 69.7 & 66.0 & 78.7 & 45.6 & \textbf{59.5} & 71.0 & 66.1 & \textbf{82.0} & \textbf{46.7} & 58.0 & 69.1 & 65.0 & 76.6 & 46.7 & 59.2 & \textbf{71.4} & \textbf{67.1} & 81.9 & 46.4 \\
Memory & 68.8 & 68.0 & 69.6 & 80.4 & 60.1 & \textbf{72.1} & \textbf{73.0} & \textbf{73.0} & \textbf{86.4} & \textbf{61.9} & 68.5 & 67.6 & 69.4 & 78.0 & 61.0 & 69.9 & 69.1 & 70.5 & 82.7 & 60.5 \\
Network & 61.2 & 73.3 & 61.9 & 86.7 & 47.3 & 61.9 & 75.5 & 65.1 & 87.9 & 47.7 & 63.1 & 76.2 & 65.0 & 87.8 & \textbf{49.3} & \textbf{63.3} & \textbf{78.4} & \textbf{67.6} & \textbf{91.6} & 48.4 \\
\hline
Average & 63.1 & 68.9 & 65.0 & 80.7 & 52.2 & \textbf{65.3} & \textbf{72.0} & \textbf{67.4} & \textbf{84.7} & 53.5 & 63.7 & 69.5 & 65.5 & 79.5 & \textbf{53.5} & 64.7 & 71.3 & 67.2 & 84.1 & 53.0 \\
\hline
\end{tabular}%
}
}
...
\begin{table}[tb]
\centering
\caption{Change detection performance: LSTM single-task versus multi-task.}
\label{tab:lstm}
\begin{adjustbox}{center, width=0.8\textwidth}
...
\begin{table}[tb]
\centering
\caption{Change detection performance: BERT single-task versus multi-task.}
...
\begin{table}[tb]
\centering
\caption{Integrated performance of root cause analysis and change detection.}
...
\subsection{Tracing}
\textcolor{red}{The trace-selection results reported below correspond to the Apache request-level workload, where LMAT is evaluated directly as an adaptive recording controller. For the Sock Shop microservice workload, our deployment study focuses instead on runtime interference on the monitored application. In that setting, we compare three system conditions: an uninstrumented baseline, tracing only, and replay-based asynchronous LMAT inference running concurrently with fresh application traffic. The final reviewer-facing protocol uses 200 virtual users, 30\,s of warm-up, 100\,s of measured load per condition, three repeats, and rotated condition order to reduce ordering bias. Because the LMAT condition replays a saved trace while the application serves a new workload, it should be interpreted as a co-located inference-overhead proxy rather than as a fully live trace-triggering deployment over the same in-flight requests.}
...
\begin{figure}[tb]
    ...
    \caption{\textcolor{blue}{Event and multi-task models' performance on adaptive tracing for various levels of noise from Sysbench set in trace data. Multi-task model shows higher efficiency in trace reduction, while keeping the miss rate and detection delay lower than the event-focused model.}}
...
\begin{figure}[tb]
    ...
    \caption{\textcolor{blue}{Impact of modeling approach on data miss rate in adaptive tracing. The multi-task model achieves higher accuracy in retaining anomalous trace data across test cases.}}
    \label{fig:missrates}
\end{figure}
...
\begin{table}[tb]
\centering
\caption{\textcolor{blue}{End-to-end component latency (ms) for LMAT under three modeling settings (batch size = 16). The Reduce column represents the model trained on the “discard-top10” event subset, which shortens sequences and reduces the output space.}}
...
\subsubsection{\textcolor{blue}{Practical deployment target}}
\textcolor{blue}{We emphasize that LMAT does not require high-end GPU clusters. We used V100s because they were available on our shared compute infrastructure; however, the LSTM variant, which in our experiments matches the Transformer variant in detection accuracy, has $\sim$1M parameters (\(\sim\)5\,MB on disk) and occupied well under 1\,GB of device memory for inference. This footprint enables deployment on modest accelerators (e.g., entry-level datacenter/consumer GPUs) and, where latency budgets permit, on \emph{CPUs only}. The model storage is a \emph{constant} overhead; LMAT’s savings accrue on the \emph{ongoing} tracing stream.}

\subsubsection{\textcolor{blue}{Cost–benefit framing}}
\textcolor{blue}{To compare compute overhead with storage savings, let:
\[
\text{SavedStorage/s} = R \times S \times \rho
\]
where \(R\) is requests/s, \(S\) is average raw trace size per request, and \(\rho\) is LMAT’s fractional reduction (e.g., \(0.706\) for 70.6\% reduction). Monthly storage saved is
\[
\Delta_{\text{storage/month}} = \text{SavedStorage/s} \times 2.592 \times 10^6\ \text{(s/month)}.
\]
Let \(C_{\text{storage}}\) denote the per-GB-month storage price and \(C_{\text{compute}}\) the monthly cost of the inference host. The break-even condition is
\[
C_{\text{compute}} \ \le \ \Delta_{\text{storage/month}} \times C_{\text{storage}}.
\]
Practitioners can instantiate \(S\) and \(C_{\text{storage}}\) from their environment and pick the appropriate inference tier:}

\textcolor{blue}{
\begin{itemize}
  \item \textbf{CPU-only tier.} For moderate \(R\) (hundreds req/s), the 1M-parameter LSTM typically fits per-core budgets; cost is amortized across existing servers.
  \item \textbf{Low-cost GPU tier.} For higher \(R\) (thousands req/s), a small GPU (well below V100 class) provides ample headroom; device power draw is modest compared to top-end accelerators.
\end{itemize}
LMAT curbs unbounded trace growth. Even when storage is “cheap”, retention, replication, backup, indexing, and downstream analytics costs scale with volume, and LMAT directly reduces all of them.}

\textcolor{blue}{
\emph{Example (our setup).} For \(2{,}000{,}000\) requests, the trace volume totals \(35\,\text{GB}\). Thus
\[
S \;=\; \frac{35}{2\times10^6}\ \text{GB/req} \;\approx\; 1.75\times10^{-5}\ \text{GB/req}\ \ (\sim 17.5~\text{KB/req}).
\]
With \(R \approx 1000\ \text{req/s}\) and \(\rho = 0.706\):
\[
\text{SavedStorage/s} \;\approx\; 1000 \times 1.75\times10^{-5} \times 0.706 \;\approx\; 1.236\times10^{-2}\ \text{GB/s}.
\]
Over a month (\(2.592\times10^{6}\) s):
\[
\Delta_{\text{storage/month}} \;\approx\; 0.01236 \times 2.592\times10^{6} \;\approx\; 32{,}024\ \text{GB}\ (\sim 31.3~\text{TB}).
\]
Using typical per-GB-per-month storage pricing\footnote{According to public cloud storage pricing (US, 2025) and on-premises shared storage cost models.}
:}
\textcolor{blue}{
\[
C_{\text{storage}} \in
\left\{
\begin{array}{ll}
\$0.012 & \text{Cloud object storage (Infrequent Access tier)}\\[4pt]
\$0.020 & \text{On-premises SAN/HDD estimate}\\[4pt]
\$0.023 & \text{Cloud object storage (Standard access tier)}
\end{array}
\right.
\]
the monthly storage savings is $\$384\text{–}\$737  $ $(32{,}024~\text{GB}\times C_{\text{storage}})$.}

\textcolor{blue}{
Our recommended deployment is the \(\sim\!1\)M-parameter LSTM (~5 MB on disk), which matched the Transformer’s accuracy/delay in our experiments and ran well under 1 GB memory. Two realistic tiers:
\begin{itemize}
  \item \textbf{CPU-only:} for hundreds of req/s, incremental cost is typically negligible on existing hosts.
  \item \textbf{Low-cost GPU (e.g., T4-class):} Capital Expenditure \(\sim\$1500\) (as listed at the time of writing\footnote{Example: PNY NVIDIA Tesla T4 16 GB GDDR6, listed at US\$1,465–1,500 on Amazon, May 2025.}) amortized over 36 months \(\Rightarrow\) \(\sim\$42\)/mo; power \(\sim70\,\text{W}\Rightarrow\sim50\,\text{kWh/mo}\approx\$6\) at \$0.12/kWh; total \(\sim\$48\)/mo.\footnote{Comparable small GPUs or consumer-grade cards yield similar costs; cloud pricing varies.}
\end{itemize}}

\textcolor{blue}{Even at conservative storage prices and without replication, monthly savings (\$384–\$737) exceed a low-cost GPU’s \(\sim\$48\)/mo; with replication or downstream indexing, the margin is substantially larger. Also, CPU-only deployments have near-zero incremental compute cost. This shows that LMAT is economically viable in settings where raw trace volume, retention windows, or downstream analytics costs dominate.}
...
\textbf{External Validity.} Our experiments are based on kernel trace data collected from an Apache2 web server under controlled scenarios. Although we augmented this dataset with additional noise types to improve generalization, the framework's effectiveness may vary across different applications, workloads, or system architectures not evaluated here. The selected noisy conditions might not encompass all real-world anomaly patterns, potentially affecting LMAT's general applicability. Future work should validate LMAT on a broader range of systems and workloads to further establish its generalizability.
...
Extensive experiments demonstrate that LMAT consistently outperforms baseline approaches across change detection, root cause analysis, and trace volume reduction. These results confirm the framework’s generalizability across a diverse set of system behaviors and noise patterns.
