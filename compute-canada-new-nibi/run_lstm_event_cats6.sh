#!/bin/bash
#SBATCH --job-name=lstm_event_cats6
...
LOG_DIR=$PROJECT/logs/lstm_event_cats6_${SLURM_JOB_ID}
...
echo "Mode         : LSTM | Event-only | cats=6"
...
python -u microservice/train_sockshop.py \
  ...
  --wandb_run_name "lstm_event_cats6_${SLURM_JOB_ID}" \
  --log_dir "$LOG_DIR" \
  --gpu 0
