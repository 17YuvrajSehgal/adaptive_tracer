#!/bin/bash
# Paper-style 5 duration bins with longer 4096-token windows.
# In code/output this is still "cats6" because category 0 is reserved for padding/non-exit.
#
# This script is intentionally separate from txt_dump_to_shards.sh so the existing
# 512-token dataset remains reproducible.
#
# With max_seq_len=4096, shard_size=5000 would create very large NPZ shards.
# We therefore reduce shard_size to 512 to keep per-shard size and peak write memory reasonable.
...
OUTPUT_DIR=$SCRATCH/micro-service-trace-data/preprocessed_lmat_kernel_cats6_seq4096
