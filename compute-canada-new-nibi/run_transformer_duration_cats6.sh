#!/bin/bash
#SBATCH --job-name=trf_duration_cats6
...
LOG_DIR=$PROJECT/logs/transformer_duration_cats6_${SLURM_JOB_ID}
...
echo "Mode         : Transformer | Duration-only | cats=6"
...
python -u microservice/train_sockshop.py \
  ...
  --wandb_run_name "transformer_duration_cats6_${SLURM_JOB_ID}" \
  --log_dir "$LOG_DIR" \
  --gpu 0
