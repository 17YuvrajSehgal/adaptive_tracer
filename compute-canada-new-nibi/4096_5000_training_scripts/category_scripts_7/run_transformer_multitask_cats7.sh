#!/bin/bash
#SBATCH --job-name=trf_multitask_cats7_4096_5000
...
DATA=$WORKROOT/micro-service-trace-data/preprocessed_lmat_kernel_cats7_seq4096_5000
LOG_DIR=$PROJECT/logs/transformer_multitask_cats7_seq4096_5000_${SLURM_JOB_ID}
...
echo "Mode         : Transformer | Multitask | paper bins=7 | seq=4096_5000"
...
loss, loss_e, loss_l = compute_loss(logits_e, logits_l, batch, device, Args, crit_e, crit_l)
...
python -u microservice/train_sockshop.py \
  ...
  --wandb_run_name "transformer_multitask_cats7_seq4096_5000_${SLURM_JOB_ID}" \
  --log_dir "$LOG_DIR" \
  --gpu 0
