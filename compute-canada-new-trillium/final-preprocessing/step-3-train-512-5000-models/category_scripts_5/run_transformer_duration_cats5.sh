#!/bin/bash
#SBATCH --job-name=trf_duration_cats5_512
...
LOG_DIR=$PROJECT/logs/transformer_duration_cats5_seq512_${SLURM_JOB_ID}
...
echo "Mode         : Transformer | Duration-only | paper bins=5 | seq=512"
...
python -u microservice/train_sockshop.py \
  ...
  --wandb_run_name "transformer_duration_cats5_seq512_${SLURM_JOB_ID}" \
  --log_dir "$LOG_DIR" \
  --gpu 0
