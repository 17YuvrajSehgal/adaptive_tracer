#!/bin/bash
#SBATCH --job-name=trf_event_cats3
...
LOG_DIR=$PROJECT/logs/transformer_event_cats3_seq4096_${SLURM_JOB_ID}
...
echo "Mode         : Transformer | Event-only | paper bins=3 | seq=4096"
...
python -u microservice/train_sockshop.py \
  ...
  --wandb_run_name "transformer_event_cats3_seq4096_${SLURM_JOB_ID}" \
  --log_dir "$LOG_DIR" \
  --gpu 0
