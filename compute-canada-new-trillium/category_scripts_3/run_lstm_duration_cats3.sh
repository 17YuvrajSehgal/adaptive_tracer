#!/bin/bash
#SBATCH --job-name=lstm_duration_cats3
...
LOG_DIR=$PROJECT/logs/lstm_duration_cats3_seq4096_${SLURM_JOB_ID}
...
echo "Mode         : LSTM | Duration-only | paper bins=3 | seq=4096"
...
python -u microservice/train_sockshop.py \
  ...
  --wandb_run_name "lstm_duration_cats3_seq4096_${SLURM_JOB_ID}" \
  --log_dir "$LOG_DIR" \
  --gpu 0
